 ...
 */
package problems.classification;

import pes.core.gp.evogpj.algorithm.ClassGPFunction;
import pes.core.gp.evogpj.algorithm.Parameters;
import pes.core.gp.evogpj.test.TestGPFunctionClassifiers;
import java.io.File;
 ...
 * @author Ignacio Arnaldo
 *
 * This is the original algorithm of Ignacio for evolving GP classifier. We have added the visusation into the algorithm
 * to monitor what classifier have been evolved. The visualisation is called in pes.core.gp.evogpj.algorithm.run_population()
 *
 * The phenotypes are points in the AUC curves. Check pes.core.gp.evogpj.evaluation.java.GPFunctionKDEJava.eval()
 *
 */

public class RunGPClassifer {
    
    public RunGPClassifer(){
    ...
    public void parseGPFunctionTrain(String args[]) throws IOException{
        ...
        if(args.length==4 || args.length==5 || args.length==6 || args.length==7 || args.length==8){
            dataPath = args[1];
            // run pes.core.gp.evogpj with standard properties
            ...
            if (args[2].equals("-minutes")) {
                ...
                if(args.length==4){
//                    props.put(Parameters.Names.FITNESS, Parameters.Operators.GPFUNCTION_JAVA);
                    ...
                    if(args[4].equals("-cpp")){
                        ...
                        if(args.length==6){
                            ...
                            if(args[6].equals("-properties")){
                                propsFile = args[7];
                                // run pes.core.gp.evogpj with properties file and modified properties
    ...
    public static void main(String args[]) throws IOException, ClassNotFoundException, InterruptedException{
        RunGPClassifer m = new RunGPClassifer();
        if (args.length == 0) {
