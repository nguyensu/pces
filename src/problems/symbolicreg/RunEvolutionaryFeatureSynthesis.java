...
package problems.symbolicreg;

import pes.core.gp.efs.evofmj.algorithm.RegressionEFM;
import pes.core.gp.efs.evofmj.evaluation.java.EFMScaledData;
import pes.core.gp.efs.evofmj.test.TestRegressionEFM;
import java.io.File;
 ...
 *
 * This is the original algorithm of Ignacio for evolutionary feature synthesis. We have added the visusation into the algorithm
 * to monitor what features have been evolved. The visualisation is called in pes.core.gp.efs.evofmj.algorithm.runEFM()
 *
 * The phenotypic characteristics of evolved features represented by the correlations of the evolved features and the labels/targers
 * on some sample subsets. Check pes.core.gp.efs.lasso.LassoFitGenerator.calculate_phenotypes() to see how phenotypes are obtained
 *
 */
public class RunEvolutionaryFeatureSynthesis {
    
    /**
     * void constructor
     */
    public RunEvolutionaryFeatureSynthesis(){
        ...
        if(args.length==4){
            ...
            if (args[2].equals("-minutes")) {
                ...
                if(numMinutes==0){
                    ...
                    EFMScaledData data = new EFMScaledData(0, 0, dataPath);
                    int numberOfOriginalFeatures = data.getNumberOfOriginalFeatures();
                    int numArchiveFeatures = 3*numberOfOriginalFeatures; //3
                    int numNewFeatures = 1*numberOfOriginalFeatures; //1
                    int maxFeatureSize = 5;
    ...
    public static void main(String args[]) throws IOException, ClassNotFoundException, Exception{
        RunEvolutionaryFeatureSynthesis m = new RunEvolutionaryFeatureSynthesis();
        if (args.length == 0) {
