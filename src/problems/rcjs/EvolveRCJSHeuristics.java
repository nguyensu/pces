...
public class EvolveRCJSHeuristics extends AdaptiveChartingGPrcjs {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        for (int i = 0; i < batchsize; i++) {
            ...
            EvolveRCJSHeuristics gp = new EvolveRCJSHeuristics(seed+i, new PrintWriter(outname + ".stat", "UTF-8"));
            EvolveRCJSHeuristics.args = args;
            EvolveRCJSHeuristics.subtitle = iconfig;
...
//                }
    ...
    public EvolveRCJSHeuristics(long seed, PrintWriter writer) throws FileNotFoundException, UnsupportedEncodingException {
        super(seed, writer);
    }
...
//        System.out.println(rsubsets.get(0).toString());
    ...
    public double[] get_phenotype(Program Prog) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int k = 0; k < Program.nComponent; k++) {
                double twt = scheduler.Serial(traindata.get(rsubsets.get(GEN).get(i)), true, k);
                mintwt = Math.min(twt, mintwt);
            }
            avgtwt += mintwt/(traindata.get(rsubsets.get(GEN).get(0)).baseline);
        }
        Prog.fit_est = avgtwt/N;

        int sample = 10;
        double[] phenotypes = new double[Program.nComponent*sample];
        for (int k = 0; k < Program.nComponent; k++) {
//            phenotypes[k] = scheduler.Serial(traindata.get(phenotype_instance_index), true, k);
            scheduler.Serial(traindata.get(phenotype_instance_index), true, k);
            for (int i = 0; i < sample; i++) {
                phenotypes[k*sample+i] = Prog.phenotype[i];
            }
        }
//        Arrays.sort(phenotypes);
        Prog.phenotype = phenotypes;
    ...
    public double fitness_function(Program Prog) {
        ...
        return result.getAverage() + 0.1*(result.getMax() - result.getMin());
    }
