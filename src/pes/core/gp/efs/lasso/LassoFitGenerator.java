...
public class LassoFitGenerator {
    ...
    private static final long MAX_AVAILABLE_MEMORY = 16L * 1024 * 1024 * 1024;
...
//        System.out.println("MAXO" + maxObservations);
    ...
    private LassoFit getLassoFit(int maxAllowedFeaturesPerModel,int DEFAULT_NUMBER_OF_LAMBDAS) {
        ...
        double[][] phenotypes = calculate_phenotypes(targets, observations, 10, 10);
    ...
    private static float[][] subtargets = null;

    private double[][] calculate_phenotypes(float[] targets, float[][] observations, int samplesize, int phenotype_dimension) {
        double[][] phenotypes = new double[observations.length][phenotype_dimension];

        // initialising samples for phenotypes calculations
        if (indices == null) {
            Random rnd = new Random(1);
            indices = new int[phenotype_dimension][samplesize];
            for (int i = 0; i < phenotype_dimension; i++) {
                for (int j = 0; j < samplesize; j++) {
                    indices[i][j] = rnd.nextInt(targets.length);
                }
            }

            subtargets = new float[phenotype_dimension][samplesize];
            for (int i = 0; i < phenotype_dimension; i++) {
                for (int j = 0; j < samplesize; j++) {
                    subtargets[i][j] = targets[indices[i][j]];
                }
            }
        }

        for (int i = 0; i < phenotype_dimension; i++) {
            float[][] subvars = new float[observations.length][samplesize];
            for (int k = 0; k < observations.length; k++) {
                for (int j = 0; j < samplesize; j++) {
                    subvars[k][j] = observations[k][indices[i][j]];
                }
                phenotypes[k][i] = MathUtil.getDotProduct(subtargets[i], subvars[k]);
            }
        }
