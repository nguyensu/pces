 ...
 *
 */
package pes.core.gp.evogpj.genotype;
...
public class TreeNode implements Serializable {
     ...
     * at this node.
     * 
     * @return representation of a boolean subtree
     * @throws GPException 
     * @see Function
     */
    public BooleanFunction generateBoolean() throws GPException {
        int arity = BooleanFunction.getArityFromLabel(label);
        Constructor<? extends BooleanFunction> f;
        try {
            f = BooleanFunction.getConstructorFromLabel(label);
            if (arity == 0) {
                return f.newInstance(label);
            }
            BooleanFunction c1 = children.get(0).generateBoolean();
            if (arity == 1) {
                return f.newInstance(c1);
            } else if (arity == 2) {
                return f.newInstance(c1, children.get(1).generateBoolean());
            }
        } catch (SecurityException e) {
            // TODO Auto-generated catch block
        } catch (NoSuchMethodException e) {
