...
/**
 * Just in time C++ code generation to evaluate the AUC of the classifiers
 * obtained with the GPFunction algorithm
 * @author Ignacio Arnaldo
 */
public class GenerateGPFunctionCpp {

	FileWriter fw;
	Expression[] expressions;

	public GenerateGPFunctionCpp(String fileName) throws IOException {
		expressions = null;
	...
	private void generateHeaders(int numberOfIndividuals, int numberOfLines, int numberOfVars, int numberOfResults) throws IOException {
		...
		fw.append("float *errorThread;\n");
                fw.append("float *minOutput;\n");
                fw.append("float *maxOutput;\n");
    ...
    private void generateFunctionIndividualScaled(int index, boolean useInts,int p) throws IOException {
        ...
        fw.append("\t\n");
        fw.append("\tminOutput[" + index + "]=minPhenotype;\n");
        fw.append("\tmaxOutput[" + index + "]=maxPhenotype;\n");
        fw.append("\n");
	...
	private void generateMain(int numberOfIndividuals, int numberOfThreads)
		...
		fw.append("\terrorThread = (float *)malloc(sizeof(float*) * " + numberOfIndividuals + ");\n");
                fw.append("\tminOutput = (float *)malloc(sizeof(float*) * " + numberOfIndividuals + ");\n");
                fw.append("\tmaxOutput = (float *)malloc(sizeof(float*) * " + numberOfIndividuals + ");\n");
		...
		for (int i = 0; i < numberOfIndividuals; i++) {
                    fw.append("\toutput << " + '"' + "ErrorIndividual " + (i + 1) + ": " + '"' + " << errorThread[" + i + "]" 
                            + " << \" \" << minOutput["+i+"] << \" \" << maxOutput["+i+"] << endl;\n");
		}
    ...
    public void readResults(ArrayList<Float> alFitness, ArrayList<Float> alMinOutputs, ArrayList<Float> alMaxOutputs) throws FileNotFoundException {
        ...
        while (sc.hasNextLine()) {
            ...
            float minTrainOutput = Float.parseFloat(minOutputS);
            alMinOutputs.add(minTrainOutput);
            
            String maxOutputS = lineSplit[4];
            float maxTrainOutput = Float.parseFloat(maxOutputS);
