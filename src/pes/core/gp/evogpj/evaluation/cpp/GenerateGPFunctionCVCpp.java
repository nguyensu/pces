...
/**
 * Just in time C++ code generation to evaluate the AUC on the validation set
 * of the classifiers obtained with the GPFunction algorithm 
 * @author Ignacio Arnaldo
 */
public class GenerateGPFunctionCVCpp {

	FileWriter fw;
	Expression[] expressions;

	public GenerateGPFunctionCVCpp(String fileName) throws IOException {
		expressions = null;
	...
	private void generateFunctionIndividualScaled(int index,float fpWeight,float fnWeight,int numLambdas) throws IOException {
		...
		fw.append("void computeCPUIndividual" + (index + 1) + "(float* dataset, int minTarget,int maxTarget, int indexIndi){\n");
	...
	private void generateThreads(int numberOfIndividuals, int numberOfThreads)throws IOException {
		...
		for (int threadID = 1; threadID <= numberOfThreads; threadID++) {
			...
			while (individualIndex < individualEndIndex) {
				fw.append("\tcomputeCPUIndividual" + (individualIndex+1)+ "(sm_dataset, minTarget,maxTarget," + (individualIndex+1)+ ");\n");
				individualIndex++;
			}
			fw.append("}\n");
			fw.append("\n");
	...
	private void generateMain(int numberOfThreads)
			throws IOException {
		...
		fw.append("\tssname << " + '"' + "tempFiles/resultsSRRocCVCpp" + '"' + "<<" + '"' + ".txt" + '"' + " ;\n");
	...
	public void runCode(String binName) {
        ...
        public void readResults(ArrayList<Float> alFitness, ArrayList<Float> alThresholds) throws FileNotFoundException{
            String fileOutput = "tempFiles/resultsSRRocCVCpp.txt";
