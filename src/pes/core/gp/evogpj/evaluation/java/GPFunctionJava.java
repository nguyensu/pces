...
public class GPFunctionJava extends FitnessFunction{
    ...
    public void eval(Individual ind) {
        ...
        List<Integer> phenotype_sample = ((CSVDataJava) data).random_sample_index;
//        double[] phenotype = new double[phenotype_sample.size()*3];
        //scale values to 0-1 range and record the phenotype of the individual
//        int count = 0;
//        for (int i = 0; i < data.getNumberOfFitnessCases(); i++) {
//            double daux = predictions[i];
//            predictions[i] = scaleValue(daux, minPhenotype, maxPhenotype);
//            if (phenotype_sample.contains(i)) {
//                phenotype[count] = predictions[i] > 0.3? 1: 0;
//                phenotype[count + phenotype_sample.size()] = predictions[i] > 0.5? 1: 0;
//                phenotype[count + 2*phenotype_sample.size()] = predictions[i] > 0.7? 1: 0;
//                count++;
//            }
//        }
...
//        ind.setPhenotype(phenotype);
        ...
        double[] phenotype = new double[(numberOfLambdas+1)*2];
        ...
        for(int l=0;l<=numberOfLambdas;l++){
            ...
            phenotype[l] = fpRatio;
            phenotype[l + numberOfLambdas] = tpRatio;
        }
        ind.setPhenotype(phenotype);
