...
/**
 * Just in time CUDA code generation evaluate GPFunction models on the validation set
 * @author Ignacio Arnaldo
 */
public class GenerateGPFunctionCVCuda{
    ...
    public GenerateGPFunctionCVCuda(String aFileName) throws IOException{
        expressions = null;
        filename = aFileName;
    } 
    ...
    private void generateMain(int numberOfLines,int numberOfVars,int numberOfResults,int maxIndividualSize,String filePathIndividuals,float fpWeight,float fnWeight,int numLambdas) throws IOException{
	...
	fw.append("\tdim3 gridRed(num_blocks_red);\n");
        ...
        fw.append("\tssname << " + '"' + "tempFiles/resultsSRRoCCVCuda" + '"' + "<<" + '"' + ".txt" + '"' + " ;\n");
	...
	fw.append("\n");
        ...
        fw.append("\t\t}\n");
        fw.append("\t\tfloat bestThreshold = endInterval;\n");
        fw.append("\t\tfloat minCost = FLT_MAX;\n");
        fw.append("\t\tfor(int l=0;l<=numberOfLambdas;l++){\n");
        ...
        fw.append("\t\t\t}\n");
        fw.append("\t\t}\n");
        fw.append("\t\toutput << \"FitnessIndividual \" << (indexIndi+1) <<\": \" << totalArea << \" \" << bestThreshold << endl;\n");
        fw.append("\t}\n");
	fw.append("\n"); 
    ...
    public void generateCode(int numberOfIndividuals,int numberOfLines,int numberOfVars,int numberOfResults,int maxIndividualSize,String filePathIndividuals,float fpWeight,float fnWeight,int numLambdas) throws IOException{
        ...
        generateMain(numberOfLines,numberOfVars,numberOfResults,maxIndividualSize,filePathIndividuals,fpWeight,fnWeight,numLambdas);
    ...
    public void readResults(ArrayList<Float> alFitness, ArrayList<Float> alThresholds) throws FileNotFoundException{
        String fileOutput = "tempFiles/resultsSRRoCCVCuda.txt";
        ...
        while(sc.hasNextLine()){
            ...
            String bestLambdaS = lineSplit[3];
            float bestLambdaAux = Float.parseFloat(bestLambdaS);
            alThresholds.add(bestLambdaAux);
        }
    }
